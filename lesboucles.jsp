...
<html>
  ...
  <body bgcolor="white">
    ...
    <p>
      <% for (int i = 1; i <= cpt; i++) { %> <%= "*****" %><br />
      <% } %>
    </p>

    <h2>Exercice 2 : Triangle rectangle gauche</h2>
    <p>
      <% for (int i = 1; i <= cpt; i++) { %> <%= "*".repeat(i) %><br />
      <% } %>
    </p>

    <h2>Exercice 3 : Triangle rectangle inversé</h2>
    <p>
      <% for (int i = cpt; i >= 1; i--) { %> <%= "*".repeat(i) %><br />
      <% } %>
    </p>

    <h2>Exercice 4 : Triangle rectangle 2</h2>
    <p>
      <% for (int i = 1; i <= cpt; i++) { %> <%= "&nbsp;".repeat(cpt - i) +
      "*".repeat(i) %><br />
      <% } %>
    </p>
    ...
    <% for (int i = 1; i <= cpt; i++) { for (int j = 1; j <= cpt - i; j++) {
    out.print("&nbsp;&nbsp;"); } for (int k = 1; k <= i; k++) { out.print("*");
    } out.println("<br />"); } for (int i = 2; i <= cpt; i++) { for (int j = 1;
    j < i; j++) { out.print("&nbsp;&nbsp;"); } for (int k = i; k <= cpt; k++) {
    out.print("*"); } out.println("<br />"); } %>

    <h2>Exercice 7 : La table de multiplication</h2>
    <% for (int i = 1; i <= cpt; i++) { out.println("
    <p>" + cpt + " x " + i + " = " + (cpt * i) + "</p>
    "); if (i == cpt) break; // Stop when i reaches the entered value } %>
